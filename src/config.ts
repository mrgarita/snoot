...
/** 天井降下アニメの所要時間（秒）。即時に下げず、この時間でスライドさせて緊張を演出する */
export const CEILING_DROP_DUR = 0.5;

/**
 * ゲームオーバー演出（骸骨化ウェーブ）の所要時間（秒）。
 * 即座に結果画面を出さず、この時間をかけて盤面を下→上に骸骨化させ、失敗の余韻をつくる。
 */
export const GAMEOVER_SEQ_DUR = 1.6;

// ---- 着弾（セット位置）の判定について ----
// 飛行中の当たり判定・着地セルの決定は最近傍セル（ボロノイ）方式に統一した。
// 弾の幅を通り道では考慮しないため調整パラメータは持たない（src/placement.ts を参照）。
