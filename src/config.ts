...
// ---- 着弾（セット位置）の判定パラメータ ----
/**
 * 衝突しきい値（セル直径に対する比）。弾中心と既存キャラ中心の距離がこの比×cell
 * 未満で衝突とみなす。半径合計は cell なので 1.0 で「ちょうど接触」。隣接キャラ間の
 * 中点は各 0.5cell のため、壁抜け（隣接キャラの間を通り抜け）を防ぐには必ず 0.5 より
 * 大きく保つこと。
 */
export const HIT_DIST_RATIO = 0.85;

/**
 * 着弾後、進行方向に沿って「奥（天井側）の空きセル」へ前進する際の許容しきい値
 * （進行方向の単位ベクトルと、隣セルへの単位方向ベクトルの内積の最小値）。
 * 大きいほど奥へ入りにくく（より手前寄り）、小さいほど奥へ入りやすい。
 * これが手前(赤)/奥(青)の撃ち分け感度を決める主パラメータ。実測で調整する。
 * v0.9.7：0.5 → 0.35、v0.9.8：まだ奥に入りにくい指摘を受け 0.35 → 0.25 に緩めた。
 */
export const SETTLE_FORWARD_MIN_DOT = 0.25;

/**
 * 前進先セルが「ねらった射線（接触点を通る進行方向の直線）」から離れてよい上限
 * （cell に対する比の垂直距離）。これを設けないと斜め狙いのとき横方向のステップでも
 * 内積が大きくなり、弾が空きセルを横滑りして盤面を大きく移動してしまう。射線が
 * 通るセルだけに前進を限定し、横滑りを防ぐ。経路上の全セルがこの範囲内に収まるため、
 * 横方向のズレは構造的に 1 セル未満に抑えられる（値を緩めても暴走しない）。
 * v0.9.7：奥のくぼみを拾いやすくするため 0.6 → 0.72 に緩めた。
 */
export const SETTLE_MAX_PERP = 0.72;

/** 前進ループの安全上限（無限ループ防止。盤面の縦セル数より十分大きければよい） */
export const SETTLE_MAX_STEPS = 20;
